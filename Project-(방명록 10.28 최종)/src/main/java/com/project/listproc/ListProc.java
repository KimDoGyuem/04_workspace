...
public class ListProc {
	...
	final int LISTCOUNTPARPAGE = 5; //리스트 표시 글 수
	...
	public void getList() {
		int startIndex = (currentPage-1)*LISTCOUNTPARPAGE;
		GetListDto gl = new GetListDto(startIndex);
		posts = mapper.getList(gl);
	}
	public int getPageCount() {
		int totalPageCount = 0;
		int count = mapper.getPostCount();
		if(count%LISTCOUNTPARPAGE==0) {
			totalPageCount = count/LISTCOUNTPARPAGE;
		}else {
			totalPageCount = count/LISTCOUNTPARPAGE+1;
		}
	...
	public void getSearchList(String word) {
		int startIndex = (currentPage-1)*LISTCOUNTPARPAGE;
		GetListDto gl = new GetListDto(word,startIndex);
